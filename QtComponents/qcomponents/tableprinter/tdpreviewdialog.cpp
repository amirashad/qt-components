...
void TDPreviewDialog::setupPage() {
    ...
    //count lines
    lines = (int) (((pageScene.height() - topMargin - bottomMargin - headerSize - footerSize) / rowHeight) - 3);
...
void TDPreviewDialog::print() {
    ...
    dialog.setWindowTitle(trUtf8("Print Document"));
    if (dialog.exec() == QDialog::Rejected) {
        return;
    }
    setupPage();
...
void TDPreviewDialog::exportPdf(const QString &filename) {
    // file save dialog
    QString dialogcaption = trUtf8("Export PDF");
...
void TDPreviewDialog::paintPage(int pagenum) {
    ...
    int borderAdjust = (int) (rowHeight / 5.0);
